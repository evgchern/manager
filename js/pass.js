...
function exit_settings() {
	...
	if((document.querySelector('#theme1').checked && localStorage.getItem('theme') !== 'light') || (document.querySelector('#theme2').checked && localStorage.getItem('theme') == 'light')) theme_change();
...
function table() {
	...
	let element = document.querySelector('.tb-content_table>.table>table');
...
function update_tr(number) {
	...
	const tableT = document.querySelector('.tb-content_table>.table>table');
...
function write_base() {
	if(document.querySelector('.tb-content_code>.write_code').style.display == `block`) {
		document.querySelector('.tb-content_code>.write_code').style.display = 'none';
		return;
	}
	document.querySelector('.tb-content_code>.write_code>.write_code__text').innerHTML = JSON.stringify(JSON.parse(localStorage.getItem('basa')));
	document.querySelector('.tb-content_code>.write_code').style.display = 'block';
}

function read_base() {
	if(document.querySelector('.tb-content_code>.read_code').style.display == `block`) {
		document.querySelector('.tb-content_code>.read_code').style.display = 'none';
		return;
	}
	document.querySelector('.tb-content_code>.read_code').style.display = 'block';
}

function read_code_base() {
	let cnt = document.querySelector('.tb-content_code>.read_code>.read_code__text').textContent;
	cnt = JSON.parse(cnt);
	basa(cnt);
	table();
}
